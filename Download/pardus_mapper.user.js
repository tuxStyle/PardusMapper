...
function getNavData(doc,enableSectorLink,enableCoordsLink) {
	...
	if (readCookie('show_over') === 'true') {
		...
		var stext = "function display_overview(uni,id) {\n"
			...
			+ "		return null;\n"
			+ "	}\n"
			+ "	var url = 'https://pardusmapper.com/info/pardus_overview_info.php';\n"
			+ "	url += '?uni=' + uni + '&id=' + id;\n"
			...
			+ "	document.getElementById('tightwad_overview').innerHTML = '<img src=\"https://pardusmapper.com/images/ajax-loader.gif\" />';\n"
		...
		overview_div.innerHTML = '<img src="https://pardusmapper.com/images/ajax-loader.gif" />';
	...
	try {
		...
		if (enableSectorLink) { doc.getElementById('status_content').getElementsByTagName('span')[0].innerHTML = '<a target="_blank" href="https://pardusmapper.com/' + readCookie('uni') + '/' + readCookie('sector') + '">' + readCookie('sector') + '</a>'; }
		...
		if (enableCoordsLink) { doc.getElementById('status_content').getElementsByTagName('span')[1].innerHTML = '<a target="_blank" href="https://pardusmapper.com/' + readCookie('uni') + '/' + readCookie('sector') + '/' + readCookie('x') + '/' + readCookie('y') + '">' + coords + '</a>'; }
...
function pardusMapper() {
	...
	if (readCookie('currentVersion') === null || readCookie('currentVersion') !== currentversion) {
		...
		if (testing) { createCookie('server',"https://pardusmapper.com/TestMap/include/"); }
		else { createCookie('server',"https://pardusmapper.com/include/"); }
	
		if (readCookie('activate') === null) { createCookie('activate',autostart); }
		if (readCookie('show_over') === null) { createCookie('show_over','false'); }

	}
	if (loc.match('pardusmapper.com')) {
		...
		if (loc.match('TestMap')) {
			...
			if (readCookie('currentVersion') < parseFloat(webVersion)) {
				...
				td.innerHTML = "<a href=\"https://pardusmapper.com/TestMap/" + uni + "/download/?dl=1\"><strong>Download<br>Script v" + webVersion + "</strong></a>";
				tr.appendChild(td);
			...
			if (readCookie('currentVersion') < parseFloat(webVersion)) {
				...
				td = doc.createElement('td');
				td.style.backgroundColor = "red";
				td.innerHTML = "<a href=\"https://pardusmapper.com/" + uni + "/download/?dl=1\"><strong>Download<br>Script v" + webVersion + "</strong></a>";
				tr.appendChild(td);
				header.appendChild(tr);
			}
		}
	...
	if (loc.match('msgframe.php')) {
		if (readCookie('activate') === 'true') {
			doc.getElementById("universe").parentNode.innerHTML += ' | <a href="javascript:activate(\'activate\');"><img border="0" id="tightwad_mapper_activate" src="https://pardusmapper.com/images/green.png" title="Tightwad\'s Pardus Mapper Active"/></a>';
		} else {
			doc.getElementById("universe").parentNode.innerHTML += ' | <a href="javascript:activate(\'activate\');"><img border="0" id="tightwad_mapper_activate" src="https://pardusmapper.com/images/red.png" title="Tightwad\'s Pardus Mapper Inactive"/></a>';
		}
		if (!window.opera) {
			if (readCookie('show_over') === 'true') {
				doc.getElementById("universe").parentNode.innerHTML += ' | <a href="javascript:activate(\'show_over\');"><img border="0" id="tightwad_mapper_show_over" src="https://pardusmapper.com/images/green.png" title="Tightwad\'s Building Overview On"/></a>';
			} else {
				doc.getElementById("universe").parentNode.innerHTML += ' | <a href="javascript:activate(\'show_over\');"><img border="0" id="tightwad_mapper_show_over" src="https://pardusmapper.com/images/red.png" title="Tightwad\'s Building Overview Off"/></a>';
			}
		}
		...
		var stext = "function activate (name) {\n"
			+ "	var z = document.getElementById('tightwad_mapper_' + name);\n"
			+ "	if (z.src.match('green')) {\n"
			+ "		document.cookie= name + '=false; path=/';\n"
			+ "		z.src = 'https://pardusmapper.com/images/red.png';\n"
			+ "	} else {\n"
			+ "		document.cookie= name + '=true; path=/';\n"
			+ "		z.src = 'https://pardusmapper.com/images/green.png';\n"
			+ "	}\n"
