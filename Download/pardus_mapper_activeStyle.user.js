// ==UserScript==
// @name	activeStype Tightwad's Copycat Mapping and Stocking Logger
// @namespace	https://pardusmapper.activestyle.ca/
// @description	Real-time Mapping and Stocking Script for the Pardus Universe
// @include	http://*.pardus.at/*
// @include	https://*.pardus.at/*
// @include	https://pardusmapper.activestyle.ca/*
// @match	http://*.pardus.at/*
// @match	https://*.pardus.at/*
// @match	https://pardusmapper.activestyle.ca/*
...
var sendDebug = '';
if (debug==true) {sendDebug="&debug=1";}
...
function overviewStats(doc) {
	try {
		...
		var postData = "lud=1&uid=" + readCookie('userid') + "&user=" + readCookie('user') + "&version=" + readCookie('currentVersion');
...
function getNavData(doc,enableSectorLink,enableCoordsLink) {
    ...
    var ships_div = doc.getElementById('otherships_content');
	if (readCookie('show_over') === 'true') {
		...
		var stext = "function display_overview(uni,id) {\n"
			...
			+ "		return null;\n"
			+ "	}\n"
			+ "	var url = 'https://pardusmapper.activestyle.ca/info/pardus_overview_info.php';\n"
			+ "	url += '?uni=' + uni + '&id=' + id;\n"
			...
			+ "			document.getElementById('tightwad_overview').innerHTML = overviewhttp.responseText;\n"
			+ "		} else {\n"
			+ "			document.getElementById('tightwad_overview').style.visibility = 'visible';\n"
			+ "		}\n"
			+ "	}\n"
			+ "	overviewhttp.send(null);\n"
			+ "}\n"
			+ "function closeWindow() {\n"
			+ "	document.getElementById('tightwad_overview').innerHTML = '<img src=\"https://pardusmapper.activestyle.ca/images/ajax-loader.gif\" />';\n"
			+ "	document.getElementById('tightwad_overview').style.visibility = 'hidden';\n"
			+ "}\n";
		s.appendChild(document.createTextNode(stext));
		doc.getElementsByTagName("head")[0].appendChild(s);
		var overview_div = document.createElement('div');
		overview_div.id = "tightwad_overview";
		overview_div.style.position = 'absolute';
		overview_div.style.border = "2";
		overview_div.style.right = '3px';
		overview_div.style.backgroundImage = 'url("https://static.pardus.at/img/std/bgoutspace1.gif")';
		overview_div.style.zIndex = '100';
		overview_div.innerHTML = '<img src="https://pardusmapper.activestyle.ca/images/ajax-loader.gif" />';
	...
	try {
		...
		if (enableSectorLink) { doc.getElementById('status_content').getElementsByTagName('span')[0].innerHTML = '<a target="_blank" href="https://pardusmapper.activestyle.ca/' + readCookie('uni') + '/' + readCookie('sector') + '">' + readCookie('sector') + '</a>'; }
		...
		if (enableCoordsLink) { doc.getElementById('status_content').getElementsByTagName('span')[1].innerHTML = '<a target="_blank" href="https://pardusmapper.activestyle.ca/' + readCookie('uni') + '/' + readCookie('sector') + '/' + readCookie('x') + '/' + readCookie('y') + '">' + coords + '</a>'; }
    ...
    if (nav_div && nav_div.style.display.match(/none/)) {
		...
		postData = "version=" + readCookie('currentVersion') + "&uid=" + readCookie('userid') + "&user=" + readCookie('user') + "&x=" + readCookie('x') + "&y=" + readCookie('y') + "&s=" + readCookie('sector') + sendDebug + "&mapdata=";
		...
		var nonBlocking = ships_div.getElementsByTagName('table');
			for (var i=0;i < nonBlocking.length;i++) {
				var nav = readCookie('userloc');
				//alert([i] + "=" + nonBlocking[i].innerHTML);
                //var fg = nonBlocking[i].getElementsByTagName('img')[0].src;
                var fg = nonBlocking[i].innerHTML;
                var opponents=fg.indexOf('opponent'); //Must be a ship, not an NPC
                if (opponents!=-1)
                {
                    fg = nonBlocking[i].getElementsByTagName('img')[0].src
                    var npcID = nonBlocking[i].getElementsByTagName('a')[0].href;
                    npcID = npcID.substring(npcID.lastIndexOf('(')+1,(npcID.lastIndexOf(',')));
                    fg = fg.substring(fg.lastIndexOf('/',fg.lastIndexOf('/')-1)+1);
                    //alert (npcID);
                    //replace ship or building with nonBlocking NPC data to avoid duplication
                    var findStr = ("<wbr>~" + parseInt(nav,10));
                    var pos = postData.indexOf(findStr);
                    var nextSqig = postData.indexOf("<wbr>",pos+1);
                    if (nextSqig == -1) {
                        var rplString = postData.substring(pos);
                    } else {
                        var rplString = postData.substring(pos,nextSqig);
                    }
                    postData = postData.replace(rplString, "<wbr>~" +  parseInt(nav,10) + "," + fg + "," + npcID);
                    if (readCookie('show_over') === 'true') {
                        nonBlocking_links[i].setAttribute('onmouseover','display_overview(\'' + uni + '\',' + parseInt(nav,10) + ')');
                        nonBlocking_links[i].setAttribute('onmouseout','closeWindow()');
                    }
                }
			}
		...
		//alert(postData);
        var lastData = readCookie('postData');
        //alert(lastData == postData);
        if (lastData != postData) {
            //alert ('sending data');
            createCookie('postData',postData);
            httpRequest = new XMLHttpComm();
            httpRequest.CallBack = httpRequest.RetrieveCommsResponse;
            //alert (doc.getElementById('aCmdAmbushSettings') == null);
            if (flyclose) {
...
function ship2OpponentCombat(doc) {
	try {
		...
		//NPC Data
        var npcID = doc.getElementsByTagName('form')[0].action;
        npcID = npcID.substring(npcID.lastIndexOf('=')+1);
        var npcStatus = doc.getElementsByName('retreat')[0].value;
        //var Dead = (npcStatus=='Return to main screen');
        //alert (npcStatus=='Return to main screen');
        //alert(npcID);
        //alert(npcID.substring(npcID.lastIndexOf('=')+1));
        postData += "<wbr>&nid=" + npcID;
        ...
        if (npcStatus=='Return to main screen') {
            postData += "<wbr>&dead=1";
        }
...
function bulletinBoard(doc) {
	...
	try {
		...
		if (readCookie('comp') !== null || readCookie('rank') !== null) {
			...
			newTd.innerHTML = "<strong><a href='https://" + readCookie('uni') + ".pardus.at/overview_stats.php'>From Tightwad's Pardus Mapper -- Please Visit the Overview Stats Page Before Visiting Builetin Boards to Log Mission Data</a></strong>";
...
function pardusMapper() {
	...
	if (readCookie('currentVersion') === null || readCookie('currentVersion') !== currentversion) {
		...
		if (testing) { createCookie('server',"https://pardusmapper.activestyle.ca/TestMap/include/"); }
		else { createCookie('server',"https://pardusmapper.activestyle.ca/include/"); }

		if (readCookie('activate') === null) { createCookie('activate',autostart); }
		if (readCookie('show_over') === null) { createCookie('show_over','false'); }

	}
	if (loc.match('pardusmapper.activestyle.ca')) {
		...
		if (loc.match('TestMap')) {
			...
			if (readCookie('currentVersion') < parseFloat(webVersion)) {
				...
				td.innerHTML = "<a href=\"https://pardusmapper.activestyle.ca/TestMap/" + uni + "/download/?dl=1\"><strong>Download<br>Script v" + webVersion + "</strong></a>";
				tr.appendChild(td);
			...
			if (readCookie('currentVersion') < parseFloat(webVersion)) {
				...
				td = doc.createElement('td');
				td.style.backgroundColor = "red";
				td.innerHTML = "<a href=\"https://pardusmapper.activestyle.ca/" + uni + "/download/?dl=1\"><strong>Download<br>Script v" + webVersion + "</strong></a>";
				tr.appendChild(td);
				header.appendChild(tr);
			}
		}
	...
	if (loc.match('msgframe.php')) {
		if (readCookie('activate') === 'true') {
			doc.getElementById("universe").parentNode.innerHTML += ' | <a href="javascript:as_activate(\'activate\');"><img border="0" id="as_tightwad_mapper_activate" src="https://pardusmapper.activestyle.ca/images/green.png" title="activeStyle Tightwad\'s Pardus Mapper Active"/></a>';
		} else {
			doc.getElementById("universe").parentNode.innerHTML += ' | <a href="javascript:as_activate(\'activate\');"><img border="0" id="as_tightwad_mapper_activate" src="https://pardusmapper.activestyle.ca/images/red.png" title="activeStyle Tightwad\'s Pardus Mapper Inactive"/></a>';
		}
		if (!window.opera) {
			if (readCookie('show_over') === 'true') {
				doc.getElementById("universe").parentNode.innerHTML += ' | <a href="javascript:as_activate(\'show_over\');"><img border="0" id="as_tightwad_mapper_show_over" src="https://pardusmapper.activestyle.ca/images/green.png" title="activeStyle Tightwad\'s Building Overview On"/></a>';
			} else {
				doc.getElementById("universe").parentNode.innerHTML += ' | <a href="javascript:as_activate(\'show_over\');"><img border="0" id="as_tightwad_mapper_show_over" src="https://pardusmapper.activestyle.ca/images/red.png" title="activeStyle Tightwad\'s Building Overview Off"/></a>';
			}
		}

		var s = document.createElement('script');
		s.type = "text/javascript";
		var stext = "function as_activate (name) {\n"
            + "	var cookiePrefix = '" + cookiePrefix + "';\n"
			+ "	var z = document.getElementById('as_tightwad_mapper_' + name);\n"
			+ "	if (z.src.match('green')) {\n"
			+ "		document.cookie= cookiePrefix + name + '=false; path=/';\n"
			+ "		z.src = 'https://pardusmapper.activestyle.ca/images/red.png';\n"
			+ "	} else {\n"
			+ "		document.cookie= cookiePrefix + name + '=true; path=/';\n"
			+ "		z.src = 'https://pardusmapper.activestyle.ca/images/green.png';\n"
			+ "	}\n"
