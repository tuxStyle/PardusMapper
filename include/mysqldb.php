<?php declare(strict_types=1);
    ...
    class mysqldb {
		...
		public function connect(): mysqli {
			...
			$dbRandy .= rand(1, 2); // Randomly append 1 or 2 to the DB_USER (or as many as you create), this was done when there was a limit to the number of queries the single user could perform
		...
		public function connect2(): mysqli { //this is/was used to split the DB connection between inbound and site usage
			$dbRandy = Settings::DB_USER;
			$dbRandy .= rand(1, 2); // Randomly append 1 or 2 to the DB_USER (or as many as you create), this was done when there was a limit to the number of queries the single user could perform
		...
		public function addBuilding($uni,$image,$id,$sb) {
			if (empty($this->db)) { $this->connect2(); }
			if ($id) {
		...
		public function pardusWHStatus($uni) {
			...
			if ($uni) {
				...
				if ($uni == 'Pegasus') {$days = $days + 3;} //Pegasus is 3 days out of sync with Orion/Artemis
